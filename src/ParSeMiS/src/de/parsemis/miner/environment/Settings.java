...
public final class Settings<NodeType, EdgeType> {
	...
	private final static <NodeType, EdgeType> Settings<NodeType, EdgeType> create(
		...
		set.factory = new HPListGraph.Factory<NodeType, EdgeType>(parser.getNodeParser(), parser.getEdgeParser());
		...
		if (set.distributionScheme.equals("threads")) {
			...
			throw new RuntimeException("Unknown distribution scheme " + set.distributionScheme);
		}
	...
	public final static <NodeType, EdgeType> Settings<NodeType, EdgeType> parse(final String[] args,
			final GraphParser<NodeType, EdgeType> parser) {
	 ...
	 */
	public final static <NodeType, EdgeType> Settings<NodeType, EdgeType> parse(final String[] args,
			final GraphParser<NodeType, EdgeType> parser, final GraphParser<NodeType, EdgeType> serializer) {
		return parse(args, parser, null, null, null, null);
	}

	public final static <NodeType, EdgeType> Settings<NodeType, EdgeType> parse(final String[] args,
			final GraphParser<NodeType, EdgeType> parser, final GraphParser<NodeType, EdgeType> serializer,
			final NodeType nnil, final EdgeType enil, final Collection<Graph<NodeType, EdgeType>> use) {
	 ...
	 * @param <EdgeType>
	 */
	public final static <NodeType, EdgeType> Settings<NodeType, EdgeType> parse2(final String[] args,
			final Collection<Graph<NodeType, EdgeType>> use) {
		return parse(args, null, null, null, null, use);
	}

	@SuppressWarnings("unchecked")
	public final static <NodeType, EdgeType> GraphParser<NodeType, EdgeType> parseFileName(final String name,
			final LabelParser<NodeType> np, final LabelParser<EdgeType> ep) {
		if (name.endsWith(".graphml") || name.endsWith(".graphml.gz") || name.endsWith(".gml")
				|| name.endsWith(".gml.gz")) {
			if (np == null || ep == null) {
				return (GraphParser<NodeType, EdgeType>) GraphmlParser.instance();
			} else {
			...
			if (np == null || ep == null) {
				return (GraphParser<NodeType, EdgeType>) new SimpleUndirectedGraphParser<Atom, Bond>(Atom.parser(),
						Bond.parser());
			} else {
				return new SimpleUndirectedGraphParser<NodeType, EdgeType>(np, ep);
			}
		} else if (name.endsWith(".sug") || name.endsWith(".sug.gz")) {
			if (np == null || ep == null) {
				return (GraphParser<NodeType, EdgeType>) SimpleUndirectedGraphParser.instance();
			} else {
				return new SimpleUndirectedGraphParser<NodeType, EdgeType>(np, ep);
			}
		} else if (name.endsWith(".sdg") || name.endsWith(".sdg.gz")) {
			if (np == null || ep == null) {
				return (GraphParser<NodeType, EdgeType>) SimpleDirectedGraphParser.instance();
			} else {
			...
			if (np == null || ep == null) {
				return (GraphParser<NodeType, EdgeType>) SubdueParser.instance();
			} else {
				return new SubdueParser<NodeType, EdgeType>(np, ep);
			}
		} else if (name.endsWith(".ccc") || name.endsWith(".ccc.gz")) {
			if (np == null || ep == null) {
				return (GraphParser<NodeType, EdgeType>) CccGraphParser.instance();
			} else {
			...
			if (np == null || ep == null) {
				return (GraphParser<NodeType, EdgeType>) new LineGraphParser<String, String>(new StringLabelParser(),
	...
	@SuppressWarnings("unchecked")
	private final static <NodeType, EdgeType> boolean parseOption(final String option, final Settings set) {
		final int index = option.indexOf('=');
		String[] temp = null;
		if (index >= 0) {
			final String[] tmp = { option.substring(0, index), option.substring(index + 1) };
			temp = tmp;
		} else {
		...
		if (temp[0].equals("--help")) {
			...
			if (set.parser == null) {
				set.parser = parseFileName(set.inputFileName,
						(set.serializer == null ? null : set.serializer.getNodeParser()),
						(set.serializer == null ? null : set.serializer.getEdgeParser()));
			}
		} else if (temp[0].equals("--outputFile")) {// done
			set.outputFileName = temp[1];
			if (set.serializer == null) {
				set.serializer = parseFileName(set.outputFileName,
			...
			if (freqs.length == 1) {
				if (freqs[0].endsWith("%")) {
					set.minProzent = Float.parseFloat(freqs[0].substring(0, freqs[0].indexOf("%"))) / 100;
				} else {
			...
			if (freqs.length == 1) {
				if (freqs[0].endsWith("%")) {
					set.maxProzent = Float.parseFloat(freqs[0].substring(0, freqs[0].indexOf("%"))) / 100;
				} else {
			...
			set.pathsOnly = temp.length == 1 || Boolean.valueOf(temp[1]).booleanValue();
		} else if (temp[0].equals("--findTreesOnly")) { // done
			set.treesOnly = temp.length == 1 || Boolean.valueOf(temp[1]).booleanValue();
		} else if (temp[0].equals("--singleRooted")) { // done
			set.singleRooted = temp.length == 1 || Boolean.valueOf(temp[1]).booleanValue();
		} else if (temp[0].equals("--connectedFragments")) { // done
			set.connectedFragments = temp.length == 1 || Boolean.valueOf(temp[1]).booleanValue();

			// embedding
		} else if (temp[0].equals("--storeEmbeddings")) { // done
			set.storeEmbeddings = temp.length == 1 || Boolean.valueOf(temp[1]).booleanValue();
		} else if (temp[0].equals("--storeHierarchicalEmbeddings")) { // done
			set.storeHierarchicalEmbeddings = temp.length == 1 || Boolean.valueOf(temp[1]).booleanValue();
		} else if (temp[0].equals("--embeddingBased")) { // done
			set.embeddingBased = temp.length == 1 || Boolean.valueOf(temp[1]).booleanValue();
			...
			if (temp[1].equals("gspan")) {
				...
				throw new IllegalArgumentException("currently supported algorithms are " + ALGORITHMS);
			}
		} else if (temp[0].equals("--zaretsky")) { // done
			set.zaretsky = temp.length == 1 || Boolean.valueOf(temp[1]).booleanValue();
		} else if (temp[0].equals("--subdue")) { // done
			set.shrink = temp.length == 1 || Boolean.valueOf(temp[1]).booleanValue();
		} else if (temp[0].equals("--closeGraph")) { // done
			set.closeGraph = temp.length == 1 || Boolean.valueOf(temp[1]).booleanValue();
			...
			set.memoryStatistics = temp.length == 1 || Boolean.valueOf(temp[1]).booleanValue();

		} else if (temp[0].equals("--naturalOrdered")) {
			set.naturalOrderedNodeLabels = temp.length == 1 || temp[1].equals("nodes") || temp[1].equals("both");
			set.naturalOrderedEdgeLabels = temp.length == 1 || temp[1].equals("edges") || temp[1].equals("both");

		} else if (temp[0].equals("--reverseOrdered")) {
			set.reverseOrderedNodeLabels = temp.length == 1 || temp[1].equals("nodes") || temp[1].equals("both");
			set.reverseOrderedEdgeLabels = temp.length == 1 || temp[1].equals("edges") || temp[1].equals("both");
		} else {
	...
	public static void printUsage(final PrintStream out) {
		...
		out.println("\t\tThe file to which the found frequent subgraphs should be written (\'-\' for stdout)");
		out.println("\t--swapFile=file (optional)");
		out.println("\t\tA file to temporarly to swap out temporary unused objects");

		out.println("");
		out.println("\t--minimumFrequency=freq (integer or percentage)");
		out.println("\t\tThe minimum frequency a fragment must have to get reported");
		out.println("\t--maximumFrequency=freq (integer or percentage) (optional)");
		out.println("\t\tThe maximum frequency a fragment can have to get reported");
		out.println("\t--minimumNodeCount=int (optional; default: 0)");
		out.println("\t\tThe minimum size in nodes a fragment must have to get reported");
		out.println("\t--maximumNodeCount=int (optional; default: 0 = all)");
		out.println("\t\tThe maximum size in nodes a fragment can have to get reported");
		...
		out.println("\t\tSpecifies that only simple paths should be found (and no trees or arbitrary graphs)");
		out.println("\t--findTreesOnly=true|false (optional; default: false)");
		out.println("\t\tSpecifies that only (undirected) trees (graphs without cycles) should be found");
		out.println("\t--singleRooted=true|false (optional; default: false)");
		out.println("\t\tSpecifies for directed graph that only single rooted ones should be found");
		out.println("\t--connectedFragments=true|false (optional; default: true)");
		out.println("\t\tSpecifies that only connected fragments should be found");

		out.println("");
		out.println("\t--storeEmbeddings=true|false (optional; default: false)");
		out.println("\t\tSpecifies that for each fragment all embeddings should be stored");
		out.println("\t--storeHierarchicalEmbeddings=true|false (optional; default: false)");
		out.println("\t\tSpecifies that for each fragment all embeddings should be stored as a hierarchical structur");
		out.println("\t--embeddingBased=true|false (optional; default: false)");
		out.println("\t\tSpecifies that the frequency should be calculated embedding based or graph based");

		out.println("");
		out.println("\t--algorithm=" + ALGORITHMS + " (optional; default: gspan)");
		out.println("\t\tSpecifies the mining algorithm to be used");
		out.println("\t--closeGraph=true|false (optional; default: false)");
		out.println("\t\tActivates fast closed mining as described for CloseGraph");
		...
		out.println("\t\tSpecifies fragment filtering to detect fragments as the algorithm of zaretsky");

		out.println("");
		out.println("Parallel options:");
		out.println("\t--distribution=local|threads|threads_np (optional; default: local)");
		out.println("\t\tThe scheme for distribution");
		out.println("\t\tlocal     : no distribution, whole serach is done in master thread");
		out.println("\t\tthreads   : distribution by local threads");
		out.println("\t\tthreads_np: distribution by local threads without pooling temporal objects");
		...
		out.println("\t\tStarts debug thread for memory measurement (takes much time)");
		out.println("\t--visualize=true|false (optional; default: false)");
		out.println("\t\tRenders the database graphs");
		out.println("\t--naturalOrdered=none|edges|nodes|boths (optional; default: none)");
		out.println("\t\tDecides if node-/edge labels are order naturally or because of its frequency");
		out.println("\t--reverseOrdered=none|edges|nodes|boths (optional; default: none)");
	...
	private Settings(final Settings set) {
		...
		this.storeEmbeddings = set.storeEmbeddings || this.embeddingBased || this.storeHierarchicalEmbeddings;
