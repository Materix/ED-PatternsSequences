...
public interface IFrequentPatternsExtractor<T extends IItem> {
	...
	default IFrequentPatternSet<T> extract(ITransactionSet<T> transactionSet, double minRelativeSupport) {
		return extract(transactionSet, (int)Math.floor(minRelativeSupport * transactionSet.size()));
	};
}
