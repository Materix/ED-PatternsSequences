...
public class StringSequenceSetReader implements ISequenceSetReader {
	...
	public ISequenceSet readSequenceSet(List<String> rows) {
		...
		Map<String, IItem> items = rows.stream()
			.map(s -> s.split("; "))
		...
		return new SequenceSet(items.values().stream().collect(Collectors.toMap(IItem::getId, i -> i)),
			rows.stream().map(s -> Arrays.stream(s.split("; "))
					.map(group -> (List<IItem>) Arrays.stream(group.split(", ")).map(items::get).distinct().collect(Collectors.toList()))
					.map(group -> (IGroup) new Group(groupIdIterator.next(), group))
					.collect(Collectors.toList())
				)
				.map(groups -> new Sequence(sequenceIdIterator.next(), groups))
